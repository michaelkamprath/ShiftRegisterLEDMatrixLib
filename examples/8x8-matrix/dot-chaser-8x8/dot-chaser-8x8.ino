...
// The Uno R4's random() implementation pulls from the Renesas hardware RNG, which
// blocks for hundreds of microseconds per call. To keep the animation smooth on that
// board we switch to a tiny xorshift PRNG, while other platforms continue to use the
// built-in random() helper so the example behaves exactly as it always has.
...
static int randomStep(void) {
  int step = 0;
  while (step == 0) {
    step = random(-1, 2); // returns -1, 0, or 1
  }
  return step;
}
#endif
